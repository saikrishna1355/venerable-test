...
function resolveExecutable(chromiumOnly: boolean): {
  path: string | null;
  tried: string[];
} {
  if (process.env.CHROME_PATH && fs.existsSync(process.env.CHROME_PATH))
    return { path: process.env.CHROME_PATH, tried: [process.env.CHROME_PATH] };
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return Response.json(
      {
        ok: false,
        message: "puppeteer-core not installed. Please add puppeteer-core.",
      },
      { status: 500 }
    );
  }
  if (!puppeteer?.launch) {
    return Response.json(
      {
        ok: false,
        message: "Detected puppeteer module without a launch() helper.",
      },
      { status: 500 }
    );
  }
  ...
  function buildArgs(withProxy: boolean): string[] {
    ...
    if (withProxy)
      argv.push(
        `--proxy-server=${process.env.PROXY_URL || "http://localhost:8081"}`
      );
    ...
    if (process.env.CHROME_DISABLE_WEB_SECURITY === "1") {
      const profile =
        ...
        `${os.tmpdir()}/seclab-puppeteer-profile`;
      argv.push("--disable-web-security", `--user-data-dir=${profile}`);
    }
    if (process.env.CHROME_EXTRA_FLAGS)
      argv.push(...process.env.CHROME_EXTRA_FLAGS.split(" "));
    return argv;
  }
    ...
    try {
      ...
      page.on("request", async (req: any) => {
        try {
          ...
          const headersObj = (req.headers ? req.headers() : {}) as Record<
            string,
            string
          >;
          const preflight =
            method === "OPTIONS" &&
            (headersObj["Access-Control-Request-Method"] ||
              headersObj["access-control-request-method"]);
          ...
          const bodyStr =
            req.postData && typeof req.postData === "function"
              ? req.postData()
              : undefined;
          ...
          const decision = await istore.hold({
            method,
            url,
            headers: headersObj,
            body: bodyStr,
          ...
          const overrides: any = {};
          if (decision.url && decision.url !== url)
            overrides.url = decision.url;
          if (decision.method && decision.method !== method)
      ...
      try {
        ...
        cdp.on("Fetch.requestPaused", async (ev: any) => {
          try {
            ...
            const bodyObj = await cdp.send("Fetch.getResponseBody", {
              requestId: ev.requestId,
            });
            const bodyStr = bodyObj.base64Encoded
              ? Buffer.from(bodyObj.body, "base64").toString("utf8")
              : bodyObj.body || "";
            const respHeadersArr = (ev.responseHeaders || []) as Array<{
              name: string;
              value?: string;
            }>;
            const respHeaderMap: Record<string, string> = {};
            for (const h of respHeadersArr) {
              if (h && h.name) respHeaderMap[h.name] = h.value ?? "";
            }
            let decision = { action: "send" } as any;
            if (istore.shouldHoldResponse(url)) {
              decision = (await istore.holdResponse({
                method,
                url,
                responseStatus: ev.responseStatusCode,
            ...
            const newHeadersMap: Record<string, string> =
              decision.responseHeaders ?? respHeaderMap;
            const newBodyStr: string = decision.responseBody ?? bodyStr;
            for (const k of Object.keys(newHeadersMap)) {
              if (k.toLowerCase() === "content-length")
                delete (newHeadersMap as any)[k];
            }
            const headersArr: Array<{ name: string; value: string }> =
              Object.entries(newHeadersMap).map(([k, v]) => ({
                name: k,
                value: String(v),
              }));
  ...
  try {
    ...
    if (navError && body.viaProxy && fallbackDirect) {
      ...
      try {
        ...
        return Response.json({
          ...
          usedProxy: false,
          fallback: "direct",
          headless: second.launchOpts.headless,
        });
      } catch (e: any) {
        return Response.json(
          {
            ok: false,
            message: "Navigation failed (proxy and direct)",
            error: String(e?.message || e || ""),
            firstError: String(navError?.message || navError || ""),
          },
          { status: 500 }
        );
      }
    ...
    return Response.json({
      ...
      launched: true,
      usingCore,
      usedProxy: !!body.viaProxy && !navError,
      headless: first.launchOpts.headless,
      execPath: first.launchOpts.executablePath || null,
      navError: navError
        ? String(navError?.message || navError || "")
        : undefined,
    });
  } catch (error: any) {
    return Response.json(
